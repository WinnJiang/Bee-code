...
%过阻尼
%导入位移数据
load displacement10.txt

N=1;
%zetaspan=0.1:0.1:1;
displacement=displacement10;
timespan=displacement(1:1001,1);
startpoint=1;
for i=1:1
    %分配各阻尼下端点位移
    endpoint=1000+startpoint;
    ...
    %设定拟合所用系数的上下限
    options.Lower = [0 -100 zeta0(i) 1e3];
    options.Upper=[100 0 zeta0(i) 1e8];
    %进行拟合
